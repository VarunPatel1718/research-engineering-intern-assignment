"""
build_network.py
Run THIRD. Builds three NetworkX graphs, computes PageRank + Louvain,
saves three JSON files for fast API serving.
"""
...
# Ensure all subreddits appear as nodes even with no crossposts
...
Gu   = G.to_undirected()
part = (best_partition(Gu) if len(Gu.edges()) > 0
        else {n: i for i, n in enumerate(Gu.nodes())})
    ...
    "nodes": [
        {
            ...
            "post_count": pc.get(n, 0),
            "community":  part.get(n, 0),
            "bloc":       BLOC.get(n, "other"),
        }
        for n in G.nodes()
    ],
    "edges": [
...
# Top 80 authors: 10+ posts OR active in 2+ subreddits
qualified = [
    a for a in asubs
    if apost[a] >= 10 or len(asubs[a]) >= 2
][:80]
...
pra   = (nx.pagerank(Ga, alpha=0.85, weight="weight")
         if Ga.edges() else {n: 0 for n in qualified})
parta = (best_partition(Ga)
         if Ga.edges() else {n: 0 for n in qualified})
...
auth_net = {
    "nodes": [
        {
            "id":             n,
            "post_count":     Ga.nodes[n]["post_count"],
            ...
            "community":      parta.get(n, 0),
        }
        for n in Ga.nodes()
    ],
    "edges": [
        {"source": u, "target": v, "weight": d["weight"], "shared": d.get("shared", [])}
...
def dbias(d: str) -> str:
    return "left" if d in LEFT_D else "right" if d in RIGHT_D else "center"

src_net = {
    "nodes": (
        [{"id": s, "type": "subreddit", "bloc": BLOC.get(s, "other")}
         for s in se["subreddit"].unique()]
        + [{"id": d, "type": "domain", "bias": dbias(d)}
           for d in se["domain"].unique()]
    ),
    "edges": se.rename(
        columns={"subreddit": "source", "domain": "target"}
